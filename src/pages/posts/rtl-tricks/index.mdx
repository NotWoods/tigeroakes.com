...
import { HelloWorld, AutoMirrorIcon, SidebarWithShadow } from './RtlDemo.tsx';
...
While working on [Microsoft Loop](https://www.microsoft.com/en-us/microsoft-loop), I introduced a [CSS custom property](https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_custom_properties) to make it easier for us to create shadows and icons that change direction in <attr title="Right to Left">RTL</attr> languages. The CSS code just looks like this:
...
This can be generalized with the `--x-direction` CSS custom property. Since the value will always be `1` or `-1`, we can directly use it as a scaling factor.

```css
.auto-mirror-icon {
  transform: scaleX(var(--x-direction));
}
```
...
In right to left layouts, we need the shadow's X offset to be `-4px` instead of `4px`. Once again we can use the `--x-direction` custom property to generalize this.

We can represent this as a math formula: _4px \* x-direction_.

In CSS, we represent this formula using the [`calc()`](https://developer.mozilla.org/en-US/docs/Web/CSS/calc) function.

```css
calc(4px * var(--x-direction))
```
