...
Primitive data types and classes marked with `@Stable` or `@Immutable` are stable. Collection classes list `List` need to be wrapped with by a class or replaced with [KotlinX immutable collections](https://github.com/Kotlin/kotlinx.collections.immutable).
