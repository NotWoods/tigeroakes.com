...
In React, a component can be marked as "pure" by wrapping it with [`React.memo`](https://reactjs.org/docs/react-api.html#reactmemo). This will prevent the component from re-rendering unless its props change.

```jsx
// Only re-renders if props.numbers is a different array
...
In Jetpack Compose, this is done automatically. Compose will automatically try to mark a function as ["skippable"](https://developer.android.com/develop/ui/compose/performance/stability#mutable-objects), which will prevent the composable from re-composing. If all the parameters of a composable are [stable](https://medium.com/androiddevelopers/jetpack-compose-stability-explained-79c10db270c8), then the compiler will mark the composable as skippable.

Primitive data types and classes marked with `@Stable` or `@Immutable` are stable. Collection classes list `List` need to be wrapped with by a class or replaced with [KotlinX immuntable collections](https://github.com/Kotlin/kotlinx.collections.immutable).
